    ...
    <>
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-16 sm:py-20 md:px-6">
        ...
        >
          ...
          <h1 className="text-4xl font-bold tracking-tight sm:text-5xl bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent dark:from-blue-400 dark:to-purple-400">
...
function FeatureCard({ emoji, label, title, blurb, links, cta, color = "blue" }) {
  ...
  return (
    <motion.div 
      variants={item}
      className={`group relative flex flex-col overflow-hidden rounded-2xl border ${colorClasses[color].border} bg-gradient-to-br ${colorClasses[color].bg} p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1 dark:shadow-none`}
    >
      {/* Icon badge */}
      ...
      <ul className="mt-6 space-y-2">
        {links.map((l) => (
          <li key={l.href}>
            <Link
              href={l.href}
              className="flex items-center justify-between rounded-lg p-3 transition hover:bg-white/50 hover:shadow-xs dark:hover:bg-neutral-800/50"
            >
      ...
      <div className="mt-6 pt-4 border-t border-neutral-200/50 dark:border-neutral-800/50">
        <Link
          href={cta.href}
          className={`inline-flex w-full items-center justify-center rounded-lg px-4 py-2.5 text-sm font-medium transition ${colorClasses[color].cta} shadow-sm hover:shadow-md`}
          aria-label={cta.text}
        >
          {cta.text}
